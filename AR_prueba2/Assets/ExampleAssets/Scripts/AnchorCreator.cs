...
public class AnchorCreator : MonoBehaviour
{
    ...
    public GameObject AnchorPrefab3
    {
        get => m_AnchorPrefab3;
        set => m_AnchorPrefab3 = value;
    }

    GameObject referencia1;
    GameObject referencia2;
    GameObject referencia3;
    ...
    void Update()
    {
        ...
        if (m_RaycastManager.Raycast(touch.position, s_Hits, TrackableType.PlaneWithinPolygon))
        {
            ...
            if (TestLocationService.state  == 1)
            {
                if (referencia1 != null)
                Destroy(referencia1);
                referencia1 = (GameObject) Instantiate(m_AnchorPrefab1, anchor.transform);
            
            }
            else if (TestLocationService.state  == 2)
            {
                if (referencia2 != null)
                Destroy(referencia2);
                referencia2 = (GameObject)
                Instantiate(m_AnchorPrefab2, anchor.transform);
            
            }
             else if (TestLocationService.state  == 3)
            {
                if (referencia3 != null)
                Destroy(referencia3);
                referencia3 = (GameObject)
                Instantiate(m_AnchorPrefab3, anchor.transform);
            }   
            if (anchor == null)
            {
